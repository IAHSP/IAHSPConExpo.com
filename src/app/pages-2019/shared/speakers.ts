export const speakerList =[  
  {
    image:`https://d3oaxt0bwkjnjn.cloudfront.net/web/images/conference/speakers/jenny-abrams.jpg`,
    name:`Jenny Abrams`,
    company:`Abrams Home Solutions`,
    topic:`Top Ten Organizing Tips to Be Successful in Business`,
    modelId:`bujiy`,
    description:`
      <p>Good organizing principals helps business owners become more efficient and productive. 
      In this presentation, Jenny shares strategies to help individuals better manage their time, 
      gain control over their paper piles, and create systems to tackle tasks and projects. Individuals 
      walk away with the steps necessary to put an organizing plan into action, resources, and 
      worksheets that are effective tools for implementation.</p>
    `
  },
  {
    image:`https://d3oaxt0bwkjnjn.cloudfront.net/web/images/conference/speakers/kristie-barnett.jpg`,
    name:`Kristie Barnett`,
    company:`The Decorologist`,
    topic:`Creating Architectural Harmony - Staging Challenging Living Areas`,
    modelId:`wathl`,
    description:`
      <p>In this presentation, you will learn how to:</p>

      <ul class="list-style-disc">
        <li>Use Psychological Staging techniques to create symmetry and flow in the most challenging living areas</li>
        <li>Arrange furnishings the right way in an open floor plan</li>
        <li>Maximize traffic flow and seating in rooms with multiple openings</li>
        <li>Conquer the dreaded corner fireplace</li>
        <li>Downplay tricky architecture such as slanted walls and ceilings</li>
        <li>Reduce buyer objections to poorly-designed remodels</li>
      </ul>
    `
  },
  {
    image:`https://d3oaxt0bwkjnjn.cloudfront.net/web/images/conference/speakers/june-carter.jpg`,
    name:`June Carter`,
    company:`Gyst Home Solutions`,
    topic:`From Stuff to Staging: Organizing and Staging in Harmony`,
    modelId:`brltr`,
    description:`
      <p>There’s Good Stuff (B52’s), Old Stuff (Garth Brooks), The Right Stuff (Bryan Ferry), 
        Stuff (Miles Davis), Ruff Stuff (AC/DC), Busted Stuff (Dave Matthews Band) and Too 
        Much Stuff (Delbert McClinton).</p>

      <p>What can we do when a home is to be staged but it has too much stuff? Adding 
        organizing to your instrument case would create an additional income stream and help 
        you to raise the bar for your business. You can increase the coloratura of your 
        staging, bring forte to your services and shift the tenor of your offerings.</p>

      <p>This presentation will allow you to make an overture to your clients to help them get 
        their stuff together before you stage the home. After this presentation you will be 
        able to provide organizing as a prelude to Home Staging.</p>

      <p>When Stuff (Third Eye Blind) gets in the way of Home Staging you can be Hot Stuff 
        (Ashlee Simpson) and add organizing to your orchestra. You can end up Lovin Stuff 
        (Muddy Waters) as your opportunities increase to make more Money, Money, Money 
        (ABBA).</p>
    `
  },
  {
    image:`https://d3oaxt0bwkjnjn.cloudfront.net/web/images/conference/speakers/marianne-cherico.jpg`,
    name:`Marianne Cherico`,
    company:`Coaching by Marianne`,
    topic:`Turn Real Estate Agents Into Raving Fans`,
    modelId:`stuhi`,
    description:`
      <p>Many stagers struggle with how to get business from Real Estate agents. They don’t know 
        what to say, how to “sell” their services and how to confidentially convey their value. 
        This presentation will dive deep into strategies to build solid relationships with Real 
        Estate agents without being sleazy or salesy or uncomfortable. In fact, my strategies 
        are “feel good” strategies that will empower you to win over Real Estate agents so that 
        they truly appreciate the benefits to working with you (and only you).</p>
    `
  },
  {
    image:`https://d3oaxt0bwkjnjn.cloudfront.net/web/images/conference/speakers/anthea-click.jpg`,
    name:`Anthea Click`,
    company:`FP Staging and Design`,
    topic:`Fret Note More! The Keys to Avoid Hiring A-Flat Employees`,
    modelId:`saspe`,
    description:`
      <p>Is your team reaching their full potential? Do they show up ready to win? Your company 
        culture has a direct impact on its performance. Learn how to attract the right people, 
        develop a winning environment, and increase your bottom line.</p>

      <p>Learning Objectives:</p>

      <ul class="list-style-disc">
        <li>Attract the ideal candidates</li>
        <li>Understand the importance of the interview process</li>
        <li>Establish your leadership role</li>
        <li>Maintain your winning team</li>
      </ul>
    `
  },
  {
    image:`https://d3oaxt0bwkjnjn.cloudfront.net/web/images/conference/speakers/liv-conlon.jpg`,
    name:`Liv Conlon`,
    company:`The Property Stagers UK`,
    topic:`Fearless - Tailor a Swift Plan to Success - 10 Steps to Build a Successful Business`,
    modelId:`jarif`,
    description:`
      <p>Learn how to build a successful business that can grow into the 7-figure realm by 
        following 10 Steps to Success.  Liv is 19 years old and has accomplished this in only 
        three years – learn what steps she took to achieve this swift success.</p>
    `
  },
  {
    image:`https://d3oaxt0bwkjnjn.cloudfront.net/web/images/conference/speakers/amber-griffiths.jpg`,
    name:`Amber Griffiths`,
    company:`Your Brand by Design`,
    topic:`3 Keys to Create your Rock Star Brand!`,
    modelId:`fiphu`,
    description:`
      <p>Overview:</p>

      <p>Increase your profits, work with people who excite you, and enjoy the genuine thrill 
        of getting paid to do what you love to do! Develop your own RockStar Brand™ that is 
        clear, cohesive, and will convert potential customers into Ideal Clients.</p>

      <p>Key takeaways will include:</p>

      <ul class="list-style-disc">
        <li>Claim your Voice!  Build your Brand and your business based on who you are in business 
          NOW - not who you started to be and not what other people think you should be.  Own your 
          voice and share your story that makes you uniquely qualified to solve their biggest 
          concern.</li>
        <li>Connect with your Fans! Clearly identify your Ideal Client - discover who they are, 
          where they are, and how to inspire and engage them with your message.  This is more than 
          just a target market – learn how to quickly identify who is meant for you, and who is 
          not.</li>
        <li>Craft your Message!  It should reflect you and engage your Ideal Client. Go beyond 
          a memorized 30 second elevator pitch – connect with them and develop a powerful and 
          profitable relationship.</li>
        <li>Bonus! Create the one-stop resource for everything about your business - your Brand 
          Soundtrack™.  Keep it clear and cohesive!</li>
      </ul>
    `
  },
  {
    image:`https://d3oaxt0bwkjnjn.cloudfront.net/web/images/conference/speakers/blair-hamaty.jpg`,
    name:`Blair Hamaty`,
    company:`Setting the Space`,
    topic:`Crazy Dreams Become Reality - Building Your Staging Empire`,
    modelId:`mosto`,
    description:`
      <p>Diversifying your business into market segments unrelated to Staging will result in 
        increased revenue and stability.  Join this session where Blair shares how he has grown 
        his company to include Staging, Design and Retail stores grossing well into 7 figures 
        annually.</p>
    `
  },
  {
    image:`https://d3oaxt0bwkjnjn.cloudfront.net/web/images/conference/speakers/kevin-htain.jpg`,
    name:`Kevin Htain`,
    company:`Identity Home Staging`,
    topic:`What REALTORS® Really Want from Stagers – Panel Discussion`,
    modelId:`phiph`,
    description:`
      <p>Understanding the mindset of a REALTOR<sup>®</sup> and what they look for and need in 
        a partnership with a professional Stager is the key to unlocking long-term partnerships.  
        Kevin will bring the Stager insight to the panel discussion to help us all understand 
        our role in the process and how to help our clients be more productive and successful.</p>
    `
  },
  {
    image:`https://d3oaxt0bwkjnjn.cloudfront.net/web/images/conference/speakers/christa-jessup.jpg`,
    name:`Christa Jessup`,
    company:`IAHSP® International Board`,
    topic:`It’s Closing Time – Panel Discussion`,
    modelId:`pusuf`,
    description:`
      <p>The struggle to get business usually boils down to lack of knowledge, confidence and 
        ability to successfully close the sale.  Christa is known as, “The Closer,” and 
        joins this panel of experts who will share their tips on how to smoothly and confidently 
        ask for the business including negotiation tips and scripting to use with clients.</p>
    `
  },
  {
    image:`https://d3oaxt0bwkjnjn.cloudfront.net/web/images/conference/speakers/leslie-and-leeann-kramer.jpg`,
    name:`Leslie and Leeann Kramer`,
    company:`Traditions Interiors`,
    topic:`Rock of Ages - Do you have something to Sell?`,
    modelId:`pustl`,
    description:`
      <p>As a business owner, it is important to think of your exit strategy is even while you 
        are focused on growth. Start planning now for your exit strategy, especially if that 
        strategy is to sell your staging business. </p>

      <ul class="list-style-disc">
        <li>How do you know if you have something to sell?</li>
        <li>Do you know the profile of your potential buyer?</li>
        <li>Do you know how to calculate the valuation of your company?</li>
        <li>What records do you need to make sure to create and save for this process?</li>
      </ul>
    `
  },
  {
    image:`https://d3oaxt0bwkjnjn.cloudfront.net/web/images/conference/speakers/joanne-lenart-weary.jpg`,
    name:`Joanne Lenart Weary`,
    company:`The JLW Company`,
    topic:`Ten Essential Steps to Avoid Achy Breaky Designs`,
    modelId:`ruvls`,
    description:`
      <p>Staging is not design...but you better know what makes a room visually appealing by 
        understanding the design riff. Blending the perfect rhythm, melody, progression, and 
        story to create a design composition that is music to the eyes every time.  Join JoAnne 
        Lenart-Weary for this upbeat approach to striking the perfect chord in your designs.</p>
    `
  },
  {
    image:`https://d3oaxt0bwkjnjn.cloudfront.net/web/images/conference/speakers/catherine-lewis-brown.png`,
    name:`Catherine Lewis Brown`,
    company:`The Successful Stager`,
    topic:`Is Your Business Out of Tune? Ensuring Pricing and Profitability are in Harmony`,
    modelId:`drina`,
    description:`
      <p>Upgrade your business, IN and OUT, so you save time, money and effort… and make more!  
        Staging business owners are often so busy working ON the part of the business they love, 
        ‘doing the staging’, that they easily put what’s happening IN their business, on the 
        back burner.  They make their clients the priority… understandable… but in the process, 
        the profitability of their business gets ignored.</p>

      <p>A stager’s business has to be in ‘tune’ so that doesn’t happen! I’m on a mission to 
        help stagers strike the right balance so the effort they’re putting into their staging 
        projects is matched by the profit they earn! No successful stager should be compromising 
        on profit money.</p>

      <p>Identify FIVE questions on what is causing your business to be less profitable than 
        it should be - and then learn the ANSWERS so your business is in ‘tune,’ and profitable.  
        Your award-winning staging is about to be matched only by your outstanding profits – 
        you are about to have the business you deserve!</p>
    `
  },
  {
    image:`https://d3oaxt0bwkjnjn.cloudfront.net/web/images/conference/speakers/melissa-marro.jpg`,
    name:`Melissa Marro`,
    company:`Rave Home Staging & Design`,
    topic:`How to build a Rock Star brand with Platinum Earnings`,
    modelId:`croyl`,
    description:`
      <p>Whether your business is in the big city, or off the country road, the right 
        strategies can cause your business to hit the top of the charts. Staging Entrepreneur, 
        Melissa Marro, showcases her hit marketing singles so your home staging company can 
        play the LP with record sales.  After building two companies into the 7-figure realm, 
        Melissa knows what it takes to hit the high notes in business.</p>
    `
  },
  {
    image:`https://d3oaxt0bwkjnjn.cloudfront.net/web/images/conference/speakers/jennie-norris.jpg`,
    name:`Jennie Norris`,
    company:`IAHSP® International Chairwoman, President & CEO, Stagedhomes.com, Owner, Sensational Home Staging`,
    topic:`Are You Running on Empty? How to Keep a Balance between Life and Work`,
    modelId:`bophe`,
    description:`
      <p>Finding a balance between life and work can be a struggle as we work to build our 
        businesses. Learn the reality and myths surrounding the idea of balance, and how to set 
        boundaries that create the balance and harmony you need to sustain growth long term.  
        Join Jennie Norris, wife, Mom of 4, entrepreneur, and marketing guru as she shares 
        lessons learned during her 17 -year Staging journey.</p>
    `
  },
  {
    image:`https://d3oaxt0bwkjnjn.cloudfront.net/web/images/conference/speakers/john-norris.jpg`,
    name:`John Norris`,
    company:`IAHSP® International Sr Exec VP, Owner, Norris4Homes`,
    topic:`What REALTORS® Really Want from Stagers and It’s Closing Time – Panel Discussions`,
    modelId:`crozu`,
    description:`
      <p>John is a seasoned, award-winning sales executive with over 25 years in high-tech, 
        Fortune 500 Sales, which earned him the nickname, “The Negotiator.”  He is also a 
        successful REALTOR® and business entrepreneur who understand how to work with Home 
        Stagers to help increase his bottom line.  John will share his wisdom and insights 
        on two panels and help us all to better understand the mindset of the REALTOR® and 
        Closing Sales.</p>
    `
  },
  {
    image:`https://d3oaxt0bwkjnjn.cloudfront.net/web/images/conference/speakers/sandra-racz.jpg`,
    name:`Sandra Racz`,
    company:`The Decorating and Staging Academy`,
    topic:`How to Hit High “C” with “E”-Decorating/Staging : Cash and Creditability awaits when you add E-Consultations and E-Design/E-Staging`,
    modelId:`fixer`,
    description:`
      <p>Clients, clients everywhere...why limit yourself to your backyard? The power of the 
        Internet allows you to decorate and stage without ever leaving your office. There are 
        many reasons why e-staging and e-decorating has become popular but what really matters 
        is how to turn it into a lucrative service. This workshop includes a step-by-step guide 
        to an actual virtual visit to a client. Watch how easy it is to gather pertinent 
        information, translate it to a successful staging or design and collect your fees.</p>

      <p>We will create a design concept and learn how to deliver a presentation package for 
        a virtual client.</p>
    `
  },
  {
    image:`https://d3oaxt0bwkjnjn.cloudfront.net/web/images/conference/speakers/sanja-radovanovic.jpg`,
    name:`Sanja Radovanovic`,
    company:`IAHSP® Europe`,
    topic:`Update on IAHSP Europe`,
    modelId:`chine`,
    description:`
      <p>Sanja and her team have done an outstanding job expanding IAHSP® throughout Europe. 
        Join Sanja for this update on what is happening in Europe and how we are making a 
        global impact.</p>
    `
  },
  {
    image:`https://d3oaxt0bwkjnjn.cloudfront.net/web/images/conference/speakers/john-reagan.jpg`,
    name:`John Reagan`,
    company:`Coldwell Banker Real Estate`,
    topic:`What REALTORS® Really Want from Stagers – Panel Discussion`,
    modelId:`tohow`,
    description:`
      <p>What do REALTORS<sup>®</sup> really want when working with a Home Stager?  This panel discussion 
        will provide valuable insights on how we can partner with REALTORS<sup>®</sup> to earn their 
        business long term, and how to be an advocate for their success.</p>
    `
  },
  {
    image:`https://d3oaxt0bwkjnjn.cloudfront.net/web/images/conference/speakers/robin-samora.jpg`,
    name:`Robin Samora`,
    company:`robinsamora.com`,
    topic:`Hit the High Note with Your Marketing`,
    modelId:`cirlb`,
    description:`
      <p>Marketing/PR expert and speaker, Robin Samora, teaches small business owners how to 
        rev up their brand online and offline, to win more attention, build leadership position 
        and get more sales. With decades of experience, she helps professionals and entrepreneurs 
        find their ideal target market, create branding, marketing and PR strategies that work 
        -- with the ultimate goal -- to bring in more business. Robin’s mission is to help 
        clients boost their brand, business and bottom line by teaching them how to cut through 
        the clutter so they can get noticed -- and make more money. She uses social media, 
        blogging, email marketing, speaking and free publicity strategies to enhance reputation 
        and credibility, positioning her clients as a “go-to” source, and customizes marketing 
        strategies according to their personal and professional goals.</p>
    `
  },
  {
    image:`https://d3oaxt0bwkjnjn.cloudfront.net/web/images/conference/speakers/sharon-schoenfeld.jpg`,
    name:`Sharon Schoenfeld`,
    company:`Remax Allegiance`,
    topic:`What REALTORS® Really Want from Stagers - Panel Discussion`,
    modelId:`fodip`,
    description:`
      <p>What do REALTORS® really want when working with a Home Stager?  This panel discussion 
        will provide valuable insights on how we can partner with REALTORS® to earn their 
        business long term, and how to be an advocate for their success.</p>
    `
  },
  {
    image:`https://d3oaxt0bwkjnjn.cloudfront.net/web/images/conference/speakers/lyn-spaeth.jpg`,
    name:`Lyn Spaeth`,
    company:`IAHSP® Executive Leadership Board, Luxe Home Staging & Design`,
    topic:`Occupied Home Staging is a Money Maker`,
    modelId:`drosl`,
    description:`
      <p>With so much focus on vacant properties with inventory and décor, Stagers may be 
        overlooking a very lucrative market segment for their business.  Occupied Home Staging 
        uses our creativity and client’s resources in ways that test our talent.  Learn how to 
        cultivate this aspect of your business where care and compassion are a necessary part 
        of working with sellers.</p>
    `
  },
  {
    image:`https://d3oaxt0bwkjnjn.cloudfront.net/web/images/conference/speakers/theresa-thomas.jpg`,
    name:`Theresa Thomas`,
    company:`Stage Right Home Staging`,
    topic:`Building a “Bridge over Troubled Waters”- Navigating through Life’s Challenges while sustaining your successful business`,
    modelId:`viteg`,
    description:`
      <p>Life happens and how we deal with the challenges we face will make or break us.  
        Join Theresa for this honest look at her journey that includes ups and downs and 
        solutions she found worked for her life to manage the everyday challenges, focusing 
        on how respond to situations and challenges to thrive or survive.</p>
    `
  },
  {
    image:`https://d3oaxt0bwkjnjn.cloudfront.net/web/images/conference/speakers/tori-toth.png`,
    name:`Tori Toth`,
    company:`Video Visibility`,
    topic:`3 Ways to Conquer Pressing Record, Build Your Visibility and Leverage Video to Gain Warm Leads`,
    modelId:`ziphl`,
    description:`
      <p>If you’re looking to reach potential clients in less time with less effort, than 
        it's time to embrace video. Winning in business today involves discovering, creating 
        and leveraging competitive advantages through the use of informative, yet entertaining 
        and actionable content. Join us to learn how to overcome your video fears and create 
        a video marketing plan you can start implementing TODAY!</p>

      <p>Tori Toth will share how to gain video visibility by discovering your “What + Why,” 
        The Process Behind Creating Successful Videos and how to Stay Consistent with Limited 
        Time. Be ready to press record by the end of this presentation. Here’s what you’ll 
        learn:</p>

      <ul class="list-style-disc">
        <li>Importance of Using Video</li>
        <li>Benefits of Video Marketing</li>
        <li>Content to Create Including 3 Videos You Must Have Now</li>
        <li>Easy Video Content System to Implement</li>
      </ul>
    `
  },
  {
    image:`https://d3oaxt0bwkjnjn.cloudfront.net/web/images/conference/speakers/kat-troulinos.jpg`,
    name:`Kat Troulinos`,
    company:`Tada Staging by Kat`,
    topic:`It's Closing Time - Overcoming Objections`,
    modelId:`drisp`,
    description:`
      <p>The struggle to get business usually boils down to lack of knowledge, confidence and 
        ability to successfully close the sale.  Overcoming Objections is a also a key part of 
        closing a sale and understanding what a client is really saying when they may say, 
        “No,” is an important part of turning a No into a Yes.  Kat joins this panel of 
        experts who will share their tips on how to smoothly and confidently ask for the 
        business including negotiation tips and scripting to use with clients.</p>
    `
  },
  {
    image:`https://d3oaxt0bwkjnjn.cloudfront.net/web/images/conference/speakers/bette-vos.jpg`,
    name:`Bette Vos`,
    company:`IAHSP® International Executive Board`,
    topic:`It's Closing Time - Overcoming Objections`,
    modelId:`xuthi`,
    description:`
      <p>The struggle to get business usually boils down to lack of knowledge, confidence 
        and ability to successfully close the sale.  Overcoming Objections is a also a key 
        part of closing a sale and understanding what a client is really saying when they 
        may say, “No,” is an important part of turning a No into a Yes.  Bette joins this 
        panel of experts who will share their tips on how to smoothly and confidently ask 
        for the business including negotiation tips and scripting to use with clients.</p>
    `
  },
  {
    image:`https://d3oaxt0bwkjnjn.cloudfront.net/web/images/conference/speakers/hayden-yates.jpg`,
    name:`Hayden Yates`,
    company:`Simply Splendid Staging & Photography`,
    topic:`Life as a Composition - Looking at what makes a composition engaging in both Staging and Photography`,
    modelId:`xajet`,
    description:`
      <p>Bring to light the elements that make a composition more dynamic, more engaging, 
        whether its a 2D image like a photograph or an actual space that is being staged. 
        Identifying these elements, whether its lighting, staging accessories, architecture 
        elements, furniture, style, color and contrast are all part of this visual language.</p>
    `
  }
];



export const isdList =[
  {
    image:`https://d3oaxt0bwkjnjn.cloudfront.net/web/images/conference/speakers/lisa-basham.jpg`,
    name:`Lisa Basham`,
    company:`Helpful Home Staging`,
    topic:`Investor Staging Designation:  If I had a Hammer - I would do a lot more than hang pictures`,
    modelId:`fewru`,
    description:`
      <p>In working with Investors, consider which updates may be needed, how to select, 
        measure, and obtain pricing and then transform the information into a quote.</p>

      <p>Topics covered:</p>

      <ul class="list-style-disc">
        <li>Building your mindset with confidence to do this work</li>
        <li>How to determine best product for client’s situation in each areas: Cabinets, 
          Counters, Flooring, Lighting and Painting</li>
        <li>Pro’s and Con’s of using a material (example: tile vs. vinyl laminate flooring)</li>
        <li>Price guidelines</li>
        <li>Guidelines on measuring for various improvements</li>
        <li>How to obtain a quote from a trade and what to look for in Sub trades quotes to you</li>
        <li>How to CYA in quoting and what to look for with Trades</li>
        <li>Overview of acting in the position of General Contractor vs. referring trades</li>
      </ul>
    `
  },
  {
    image:`https://d3oaxt0bwkjnjn.cloudfront.net/web/images/conference/speakers/sheila-dragon.jpg`,
    name:`Sheila Dragon`,
    company:`Dragon Design Co.`,
    topic:`Flipping Houses is not the time to be a Gambler`,
    modelId:`moswl`,
    description:`
      <p>Sheila is a Home Stager who has transitioned into a property Investor/Flipper, raising Capital 
        to generate funds to allow her to purchase properties, fix and resell them.  She understands 
        the mindset of investors and how Home Stagers can work together for success and be of benefit 
        to Investors.  Understanding where we can contribute in the process and benefits we offer will 
        help us understand how to market to capture this business segment.</p>
    `
  },
  {
    image:`https://d3oaxt0bwkjnjn.cloudfront.net/web/images/conference/speakers/becky-harmon.jpg`,
    name:`Becky Harmon`,
    company:`Transformations Home Staging & Design`,
    topic:`Material Trends to Make Your House Sing`,
    modelId:`mixof`,
    description:`
      <p>Becky will share all about the current material trends and how to incorporate them into a 
        property renovation.  She will cover everything you need to know about:</p>

      <ul class="list-style-disc">
        <li>Best sources for referral fees</li>
        <li>Carpet styles and fiber, pads, applications, pros and cons, trends</li>
        <li>Engineered wood styles and materials, applications, trends</li>
        <li>Laminate vs. Luxury Vinyl Planking, pros and cons</li>
        <li>Wood-look tile, pros and cons</li>
        <li>Porcelain vs. ceramic tile</li>
        <li>Best backsplash options and why</li>
        <li>Granite vs. Quartz</li>
        <li>Best practices for small renos</li>
        <li>Liability waivers</li>
      </ul>
    `
  },
  {
    image:`https://d3oaxt0bwkjnjn.cloudfront.net/web/images/conference/speakers/sandra-holmes.jpg`,
    name:`Sandra Holmes`,
    company:`IAHSP® International Executive Board, Luxe Home Staging & Design`,
    topic:`Work It 9-5 to Cultivate Loyal Cliente`,
    modelId:`dufrl`,
    description:`
      <p>Working with Investors starts with getting the business. What steps can you take to market 
        your services to this segment of the industry and how to you keep their business loyalty 
        long term?  Find out from Sandra as she shares the importance of positioning your company 
        and services in the market place to attract investor clients.</p>
    `
  }
]